...
var BranchUpdater = function(opts) {
    ...
    var cloneOrUpdateBranches = function(branches, callback){
        ...
        var branchInfo = { repoName: branch.repoName, branchName: branch.branchName, branchDir: branchDir };
        if(updated)
        {    
            branchInfo.branchHash = newHash;

            trogdor.emit('branchUpdate', branchInfo)
            .then(function(){
                shell.cd(branch.branchName);
                return trogdor.emit('branchVerify', branchInfo);
            })
            .then(function(){
                setTimeout(function(){
                    // reset the cwd
        ...
        {
            trogdor.emit('branchVerify', branchInfo)
            .then(function(){
                setTimeout(function(){
                    shell.cd(cwd);
                    cloneOrUpdateBranches(branches, callback);
    ...
    var cleanupRepo = function(repoList, finalCallback){
        ...
        console.log("BranchUpdater#cleanupRepo existingBranchNames");
        ...
        promiseChain.then(function(){
            shell.cd('..');
            cleanupRepo(repoList, finalCallback);
        });
    };
